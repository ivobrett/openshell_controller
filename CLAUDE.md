...
| **`docs/runbooks/fresh-install-known-issues.md`** | After provisioning a fresh cloud AgentGateway box, or when one behaves oddly before its first sandbox exists. Covers the "Unknown gateway 'nemoclaw'" empty state (FIXED), the "endpoint ambiguity" grant failure and its `tls` exception (FIXED), the standalone-NemoClaw-package pin that was stranded at v0.0.73 (FIXED 2026-08-23), and one remaining OPEN item: the `host.docker.storage_incompatible` warning. |
