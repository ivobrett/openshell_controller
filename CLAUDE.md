...
| **`docs/runbooks/fresh-install-known-issues.md`** | After provisioning a fresh cloud AgentGateway box, or when one behaves oddly before its first sandbox exists. Covers the "Unknown gateway 'nemoclaw'" empty state (FIXED), the "endpoint ambiguity" grant failure and its `tls` exception (FIXED), plus two OPEN items: the `host.docker.storage_incompatible` warning and the standalone NemoClaw package still pinned at v0.0.73. |
