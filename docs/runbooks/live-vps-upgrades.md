...
- **Since NemoClaw v0.0.80 the installer upgrades running sandboxes by
  design** (policy change accepted by the operator 2026-07-11).
  install.sh runs `upgrade-sandboxes --auto` after the strict
  pre-upgrade backup: any RUNNING sandbox whose agent version or
  NemoClaw build fingerprint is stale is rebuilt automatically —
  container recreated on the new image, state restored from the
  validated backup taken moments earlier. Expect a brief interruption
  per rebuilt sandbox. The rebuild preflight fails safe ("Sandbox is
  untouched — no data was lost") when host checks fail. Sandboxes with
  matching agent versions and no fingerprint drift (incl. legacy
  pre-fingerprint rows) are left alone. *Pre-0.0.80 semantics (never
  touch running sandboxes) no longer apply once you bump past 0.80.*
- Files flagged "User-managed files will not be preserved" in the
  rebuild output (seen: `.hermes/.env`) are regenerated by onboarding.
  **Messaging tokens that lived raw in a pre-0.18 `.hermes/.env` are
  LOST by the rebuild** (hit 2026-07-11: telegram went silent after the
  hermes 0.18 rebuild). Do NOT copy the raw token back — Hermes 0.18
  refuses raw secrets in `.env` (secret-boundary check), and ANY manual
  `.env` edit trips the config-integrity guard, quarantining the next
  gateway respawn ("HERMES_MCP_CONFIG_DRIFT ... rebuild the sandbox").
  The sanctioned repair: `nemoclaw <name> channels add telegram` with
  `TELEGRAM_BOT_TOKEN` in the CLI's env (registers the credential with
  the gateway + registry so it survives all future rebuilds), then
  `nemoclaw <name> rebuild --yes`. Non-secret env like
  `TELEGRAM_HOME_CHANNEL` cannot be hand-added either — configure via
  the channel config, not `.env`.
- After a rebuild, the messaging pairing store is restored AFTER the
  gateway boots, so the fresh gateway may treat already-approved users
  as unknown ("Unauthorized user" + a new pairing prompt) while
  `hermes pairing list` shows them approved. One gateway restart
  (pkill the in-sandbox `gateway run` process; nemoclaw-start respawns
  it, then `nemoclaw <name> recover`) reconciles memory with the
  restored store.
- **An upgrade must never require deleting a running sandbox.** If a
  procedure would, stop and re-plan. (Automatic rebuild-with-restore is
...
| **D. Stale onboarding session** — `Previous onboarding session failed. Re-run with --fresh...` | A previous onboarding attempt died mid-flight | `HOME=/root nemoclaw onboard --fresh --non-interactive ...` to discard, or `--resume` to continue it. The versioned installer does not pass either flag, so clear the session before re-running it (`mv ~/.nemoclaw/onboard-session.json{,.bak-$(date +%s)}` also works). |
| **E. Rebuild preflight: primary forward owned by another sandbox** (v0.0.80+) — `Rebuild preflight failed ... Dashboard port 18789 belongs to sandbox '<other>'` | `upgrade-sandboxes --auto` rebuilds a sandbox whose replacement onboarding must claim host port 18789, but that singleton "primary dashboard" forward currently points at a different sandbox (e.g. after a manual `nemoclaw <other> recover`) | Fails safe — the sandbox is untouched. `openshell forward stop 18789 <other>` then re-run the installer; onboarding claims 18789 for the rebuilt sandbox. The other sandbox's forward re-spins on its next `recover`/`dashboard/open`; controller dashboard tunnels (2093x) are unaffected. Hit 2026-07-11. |
