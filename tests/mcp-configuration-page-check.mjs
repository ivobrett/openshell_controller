...
const sidebarPath = path.join(root, 'app/components/shell/nav.ts')
const sandboxListPath = path.join(root, 'app/components/sandbox/SandboxMcpAccess.tsx')
const pagePath = path.join(root, 'app/(shell)/mcp/page.tsx')
...
assert.match(interSandboxChatOpenClawDispatchSource, /NEMOCLAW_BIN[\s\S]*\["sandbox", "gateway", "restart", sandboxName\]/, 'Inter-Sandbox Chat dashboard recovery must use NemoClaw native agent lifecycle')
assert.doesNotMatch(interSandboxChatOpenClawDispatchSource, /\/usr\/local\/bin\/openclaw gateway run/, 'Inter-Sandbox Chat must not manually launch OpenClaw in a managed sandbox')
...
assert.match(privilegedFilesSource, /\["sandbox", "exec", "-n", sandboxName/, 'privileged sandbox file writes must use the OpenShell sandbox exec path')
assert.match(privilegedFilesSource, /hostCommandEnv/, 'privileged sandbox file writes must use the shared OpenShell command environment')
...
assert.match(brokerUrlSource, /host\.docker\.internal/, 'Sandbox MCP broker handoff must use the portable Docker host alias without inspecting a legacy cluster container')
assert.doesNotMatch(brokerUrlSource, /discoverOpenShellDockerGateway|OPEN_SHELL_CONTAINER|docker.*inspect/, 'Sandbox MCP broker handoff must not depend on obsolete cluster containers')
assert.match(brokerUrlSource, /LOCAL_HOSTNAMES/, 'Sandbox MCP broker handoff must rewrite local-only browser origins')
assert.match(brokerUrlSource, /shouldUseProxyWrappedBrokerUrl/, 'Sandbox MCP broker handoff must make proxy-wrapped URLs opt-in')
assert.match(brokerUrlSource, /if \(!shouldUseProxyWrappedBrokerUrl\(\)\) return hostBrokerUrl/, 'Sandbox MCP broker handoff must prefer the direct gateway URL by default')
...
assert.doesNotMatch(sandboxOpenClawMcpConfigSource, /\/proc\/[[]0-9[]][*]|openclaw gateway run/, 'MCP config changes must not kill or launch OpenClaw processes outside NemoClaw lifecycle ownership')
...
assert.match(sandboxPermissionsSource, /"--input-type=module"/, 'MCP broker network sync probe must support top-level await')
assert.match(sandboxPermissionsSource, /await fetch\(/, 'MCP broker network sync probe must wait for the policy-generating request')
assert.match(sandboxPermissionsSource, /extractProxiedTargetUrl/, 'MCP broker network sync must inspect proxy-wrapped target URLs')
assert.match(sandboxPermissionsSource, /const proxiedTarget = extractProxiedTargetUrl\(brokerBaseUrl\)/, 'MCP broker network sync must derive the target host behind the sandbox HTTP proxy')
assert.match(sandboxPermissionsSource, /if \(proxiedTarget\) addEndpointCandidate\(candidates, proxiedTarget\)/, 'MCP broker network sync must approve the proxied broker target host')
...
// The per-sandbox MCP access UI moved from SandboxList into the MCP tab
// component app/components/sandbox/SandboxMcpAccess.tsx (Phase 4). The card MCP
// indicator moved to app/components/sandbox/CapabilityChips.tsx.
assert.match(sandboxListSource, /"blocked"/, 'Sandbox MCP tab must show per-sandbox allow/block state')
...
const capabilityChipsSource = await readFile(path.join(root, 'app/components/sandbox/CapabilityChips.tsx'), 'utf8')
assert.match(capabilityChipsSource, /MCP capabilities/, 'Sandbox rows must show a per-sandbox MCP capability indicator')
