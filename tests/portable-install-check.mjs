...
assert.match(installSource, /upsert_env "OPENSHELL_CONTROL_CREATE_GPU_MODE" "none"/, 'installer must default dashboard-created sandboxes to no GPU passthrough')
...
assert.match(versionedInstallSource, /OPENSHELL_VERSION="\$\{OPENSHELL_VERSION:-v0\.0\.116\}"/, 'versioned installer must default to the OpenShell release required by pinned NemoClaw v0.0.128 (blueprint min==max==0.0.116)')
assert.match(versionedInstallSource, /NEMOCLAW_INSTALL_REF="\$\{NEMOCLAW_INSTALL_REF:-\$\{NEMOCLAW_INSTALL_TAG:-v0\.0\.128\}\}"/, 'versioned installer must pin NemoClaw to tag v0.0.128')
...
assert.match(versionedInstallSource, /OPENCLAW_VERSION="\$\{OPENCLAW_VERSION:-2026\.9\.1\}"/, 'versioned installer must default to NemoClaw v0.0.128\'s OpenClaw build target')
...
assert.match(versionedInstallSource, /Check NEMOCLAW_INSTALL_REF\/NEMOCLAW_INSTALL_TAG/, 'missing NemoClaw ref errors must tell operators which pinning variables to check')
assert.match(versionedInstallSource, /set this on bash or export it first/, 'versioned installer help must explain curl-pipe ref pinning')
assert.match(versionedInstallSource, /curl -fsSL https:\/\/www\.nvidia\.com\/nemoclaw\.sh \| NEMOCLAW_INSTALL_TAG=v0\.0\.56 bash/, 'versioned installer help must include the safe curl-pipe pinning placement')
assert.match(versionedInstallSource, /OPENSHELL_VERSION="\$OPENSHELL_VERSION" sh/, 'versioned installer must pass the selected OpenShell version to upstream install.sh')
